...
BOOST_AUTO_TEST_CASE(advanced)
{
	...
	expr = ConfigCompiler::CompileText("<test>", R"(regex_submatch("f(o)o", "foo") == [ "foo", "o" ])");
	BOOST_CHECK(expr->Evaluate(frame).GetValue());

	expr = ConfigCompiler::CompileText("<test>", R"(var nl = regex_submatch("f(o)o", "bar"); nl == null && typeof(nl) == Object)");
	BOOST_CHECK(expr->Evaluate(frame).GetValue());
